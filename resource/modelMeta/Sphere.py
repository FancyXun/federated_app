...
class Sphere:
    ...
    def build(self):
        ...
        res_h_33 = prelu(res_conv_33, name='res_act_33')
...
embedding, loss, optimizer, accuracy = model.build()
print(embedding)
training_epochs = 5
...
tf.compat.v1.train.write_graph(sess.graph, "./", "sphere.pb", as_text=False)
print("save trainable variable...")
trainable_var = tf.trainable_variables()
with open("sphere_trainable_var.txt", "a+") as f:
    variables_sum = 0
...
with open("sphere_trainable_init_var.txt", "a+") as f:
    variables_sum = 0
    for var in trainable_var:
        accumulate = 1
        for i in range(len(var.shape)):
...
with open("sphere_feed_fetch.txt", "a+") as f:
