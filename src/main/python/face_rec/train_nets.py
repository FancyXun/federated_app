...
def get_parser():
    ...
    parser.add_argument('--eval_datasets', default=['lfw',
                                                    #'cfp_ff', 'cfp_fp', 'agedb_30'
...
def get_data(data_block):
    ...
    x = np.zeros(shape=(size, 112, 112, 3), dtype=np.float32)
    s = 0
    for path in data_path:
        im = Image.open(str(path)).resize((112, 112))
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        with open("./graph/" + "mobileFaceNet" + ".txt", "w") as f:
            variables_sum = 0
        ...
        with open("./graph/" + "mobileFaceNet_init" + ".txt", "w") as f:
            variables_sum = 0
            for var in global_var:
                ...
                for i in range(len(var.shape)):
                    accumulate = var.shape[i] * accumulate
                variables_sum = accumulate + variables_sum
                if 'MobileFaceNet' in var.initial_value.op.name:
