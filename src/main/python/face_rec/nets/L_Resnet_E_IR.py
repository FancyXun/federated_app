...
def residual_unit_v3(data, out_filter, stride, dim_match, trainable, name):
    ...
    conv1 = convolution(bn1_pad, group=1, shape=shape, strides=[1, 1], padding='VALID', trainable=trainable,
                        name=name + '_conv1')
    ...
    conv2 = convolution(relu1_pad, group=1, shape=shape, strides=stride, padding='VALID', trainable=trainable,
                        name=name + '_conv2')
    ...
    if dim_match:
        ...
        conv1sc = convolution(data, group=1, shape=shape, strides=stride, padding='VALID', trainable=trainable,
                              name=name + '_conv1sc')
...
def prelu(input, trainable, name):
    gamma = tf.get_variable(initializer=tf.constant(0.25, dtype=tf.float32, shape=[input.get_shape()[-1]]),
                            trainable=trainable, name=name + "_gamma")
...
def batch_normalization(input, trainable, name, **kwargs):
    ...
    moving_mean = tf.get_variable(shape=shape, initializer=tf.zeros_initializer, trainable=trainable,
                                  name=name + "_mean")
    moving_variance = tf.get_variable(shape=shape, initializer=tf.ones_initializer, trainable=trainable,
                                      name=name + "_var")
    offset = tf.get_variable(shape=shape, initializer=tf.zeros_initializer, trainable=trainable, name=name + "_bias")
    scale = tf.get_variable(shape=shape, initializer=tf.ones_initializer, trainable=trainable,
                            name=name + "_scale") if name != 'fc1' else None
...
def convolution(input, group, shape, trainable, name, **kwargs):
    ...
    if name.endswith('_sc'):
        b = tf.get_variable(initializer=tf.truncated_normal(input.get_shape().as_list()[-1::], stddev=0.1),
                            trainable=trainable, name=name + "_bias")
        layer = layer + b
    return layer
...
def resnet(inputs, w_init, units, num_stages, filter_list, trainable, reuse=False):
    ...
    with tf.variable_scope(variable_scope, reuse=reuse):
        ...
        net = convolution(net, group=1, strides=[1, 1], shape=[3, 3, 3, 64], padding='VALID', trainable=trainable,
                          name='conv0')
        ...
        for i in range(num_stages):
            body = residual_unit(body, filter_list[i + 1], (2, 2), False, trainable=trainable,
                                 name='stage%d_unit%d' % (i + 1, 1))
            for j in range(units[i] - 1):
                body = residual_unit(body, filter_list[i + 1], (1, 1), True, trainable=trainable,
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            trainable_placeholder = tf.placeholder_with_default(tf.constant(False, dtype=tf.bool), shape=None,
                                                                name='trainable')
